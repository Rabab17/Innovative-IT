  ...
  return (
    <div className="bg-white dark:bg-gray-950 min-h-screen" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <section className="py-16"> {/* Added py-16 for vertical padding */}
        <div className="container mx-auto px-4">
          {/* Filters */}
          <div className={`mb-12 space-y-6 ${language === 'ar' ? 'text-right' : ''}`}> {/* Add text-right here for the overall container */}
            {/* Service Type Filter */}
            <div className={`flex flex-wrap gap-3 ${language === 'ar' ? 'justify-end flex-row-reverse' : ''}`}>
              {(language === 'ar' ? [...serviceTypes].reverse() : serviceTypes).map((type) => (
                <Button
                  ...
                  className={`px-4 py-2 rounded-full transition-all duration-300 hover:scale-105 ${ // Increased padding for better button appearance
                    language === 'ar' ? 'font-arabic' : ''
                  } ${
                    selectedServiceFilter === type.id
                      ? 'bg-primary text-white dark:bg-primary-dark dark:text-white' // Adjusted for dark mode selected state
                      : 'bg-gray-100 text-gray-800 hover:bg-primary/10 hover:text-primary dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-primary-dark/80 dark:hover:text-white' // Adjusted for dark mode unselected state
                  }`}
                >
                  {type.name}
                </Button>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 hover:scale-[1.02] group">
                  <div className="relative overflow-hidden">
                    ...
                    <div className="absolute top-4 right-4">
                      <span className="bg-white/90 text-primary px-3 py-1 rounded-full text-sm font-medium dark:bg-gray-700 dark:text-primary-lighter"> {/* Adjusted for dark mode */}
                  ...
                  <div className="p-8">
                    <h3 className={`text-2xl font-bold mb-3 group-hover:text-primary transition-colors duration-300 text-gray-900 dark:text-white ${ // Adjusted for dark mode
                      language === 'ar' ? 'font-arabic text-right' : ''
                    }`}>
                      {project.title}
                    </h3>

                    <p className={`text-gray-600 dark:text-gray-300 mb-6 ${ // Adjusted for dark mode
    ...
    {language === 'ar' ? (
      ...
      <>
        ...
        <span>{project.teamSize} members</span>
                    ...
                    {/* Technologies */}
                    <div className="mb-6">
                      <h4 className={`font-semibold mb-2 text-gray-800 dark:text-white ${language === 'ar' ? 'font-arabic text-right' : ''}`}> {/* Adjusted for dark mode */}
                      ...
                      <div className={`flex flex-wrap gap-2 ${language === 'ar' ? 'justify-end' : ''}`}>
                        {project.technologies.map((tech, techIndex) => (
                          <span
                            key={techIndex}
                            className="bg-primary/10 text-primary px-2 py-1 rounded-full text-xs font-medium dark:bg-primary-dark/20 dark:text-primary-lighter" // Adjusted for dark mode
                          >
                            {tech}
                          </span>
                        ))}
                    ...
                    <div className="mb-6">
                      <h4 className={`font-semibold mb-3 text-gray-800 dark:text-white ${language === 'ar' ? 'font-arabic text-right' : ''}`}> {/* Adjusted for dark mode */}
                      ...
                      <div className="project-features-list">
                        {project.features.map((feature, featureIndex) => (
                          ...
                          >
                            ...
                            <span className="project-feature-text text-gray-700 dark:text-gray-300"> {/* Adjusted for dark mode */}
                              {feature}
                            </span>
                          </div>
                        ))}
                    ...
                    {/* Results */}
                    <div className="mb-6 p-4 bg-gradient-to-r from-primary/5 to-accent/5 rounded-xl dark:bg-gray-700"> {/* Adjusted dark mode background */}
        ...
        <span>Achieved Results:</span>
                      ...
                      <div className="grid grid-cols-1 gap-2 text-sm">
                        {Object.values(project.results).map((result, resultIndex) => (
                          ...
                          >
                            <span
                              className={`text-gray-600 dark:text-gray-300 ${ // Adjusted for dark mode
                                language === 'ar' ? 'font-arabic text-right' : ''
                              }`}
                            >
                              • {result}
          ...
          {filteredProjects.length === 0 && (
            <div className="text-center py-16">
              <p className={`text-gray-500 text-lg dark:text-gray-400 ${language === 'ar' ? 'font-arabic' : ''}`}> {/* Adjusted for dark mode */}
